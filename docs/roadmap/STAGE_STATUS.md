...
- `customer-reply-attention-unread-foundation` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_217_CUSTOMER_REPLY_ATTENTION_UNREAD_FOUNDATION.md`: use existing `replyCount` and `attentionLevel: 'reply'` contracts for channel/direct replies to the current user's messages, keep mention priority higher than reply, preserve active-read and mute behavior, and avoid DB schema/migration, new event types, scroll-to-original, and reply block redesign
...
- implement Segment 217 reply attention unread foundation, then smoke 3-user channel reply attention and direct reply attention before any reply navigation polish
...
- next focused implementation target after Segment 216 verification: reply attention/unread foundation, unless manual smoke exposes another reply foundation blocker
