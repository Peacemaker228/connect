...
- `customer-unread-realtime-idempotency-reconnect-fix` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_193_CUSTOMER_UNREAD_REALTIME_IDEMPOTENCY_RECONNECT_FIX.md`: local two-browser testing exposed unread cache drift after idle/reconnect, so realtime unread must dedupe by message id and reconcile from backend summary before global unread/server badges are expanded

Next likely work:
- staging operator migration-state inspection and unread two-user smoke
- `customer-unread-realtime-idempotency-reconnect-fix`
- after realtime idempotency passes, continue to `customer-global-unread-summary-and-server-badges`
