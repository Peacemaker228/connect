...
- run two-user smoke for unread sound/mute behavior
- next scoped notification follow-up: `customer-unread-per-chat-sound-mute-controls`, so users can mute sound for specific noisy channels or direct conversations while preserving visual unread badges/counts
