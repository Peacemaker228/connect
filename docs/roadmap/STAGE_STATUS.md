...
- `customer-chat-anchor-context-history-navigation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_220_CUSTOMER_CHAT_ANCHOR_CONTEXT_HISTORY_NAVIGATION.md`: Segment 219's overlay is replaced by anchored history mode for unloaded reply targets, context endpoints now return `olderCursor` / `newerCursor` and accept `direction=older|newer` for bounded adjacent history reads, initial target context uses the normal bounded page size on each side, normal latest infinite-query behavior remains intact, latest/anchored ranges auto-fill underfilled viewports with bounded follow-up reads, older/newer loading triggers near the viewport boundary and preserves viewport position after prepending rows, reply targets use deterministic container-relative centering with fake-smooth destination transitions for far/newly loaded jumps, latest-mode auto-scroll/forced-bottom scrolling is disabled while anchored, anchored history shows the welcome/start state at the beginning, anchored rows/reply previews keep realtime update/delete patching, and a sticky down control appears whenever the user is away from the live bottom; virtualization, reply visual redesign, reply attention/unread changes, DB schema/migrations, auth/storage/media/WebRTC, realtime transport hardening, deep links, and thread UI remain out of scope
