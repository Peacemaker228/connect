...
- auto-update implementation is proven for packaged staging `0.0.3 -> 0.0.4`, updater lifecycle hardening smoke passed, staging was rolled forward to `0.0.5`, and update-ready UX smoke passed through `0.0.6 -> 0.0.7`; the process is still manual and not backed by CI/CD;
- update discovery no longer depends only on the desktop account menu after Segment 231B; the compact visible `Restart` action was validated in an installed staging desktop update flow;
