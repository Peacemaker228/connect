...
- `customer-reply-target-context-navigation` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_219_CUSTOMER_REPLY_TARGET_CONTEXT_NAVIGATION.md`: this next reply slice should replace the Segment 218 unloaded-original no-op with bounded backend/SDK/UI context loading, then scroll/highlight the target; loaded targets keep Segment 218 behavior, soft-deleted targets should be navigable as safe fallback rows, and reply visual redesign, new reply attention behavior, DB schema/migrations, realtime events, auth/storage/media changes, and staging/prod migration execution remain out of scope
...
- implement Segment 219 reply target context navigation so reply previews can load/navigate to originals outside the currently loaded range without an unbounded "load previous" loop
