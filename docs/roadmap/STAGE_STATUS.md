...
- `customer-mentions-attention-foundation` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_204_CUSTOMER_MENTIONS_ATTENTION_FOUNDATION.md`: the first mentions slice should add metadata-backed `@user` / `@all` channel-message attention, persisted mention targets, server-side target resolution, mention rendering, and correct per-recipient mention counts while keeping normal unread, reply-to-message, link rendering, message copy, storage/auth/media changes, and broad realtime transport hardening separate
...
- implement `customer-mentions-attention-foundation` as the next product slice after the already-pending smokes are either completed or explicitly accepted as pending
