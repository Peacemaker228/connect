...
- `customer-chat-initial-scroll-stability-fix` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_222_CUSTOMER_CHAT_INITIAL_SCROLL_STABILITY_FIX.md`: chat entry now has a single initial-scroll owner before normal read/scroll side effects are enabled, unread chats can perform bounded older-page loading before settling on the first loaded unread non-own message, chats without unread settle at latest bottom, initial latest windows load bounded older pages when rendered rows are shorter than the viewport, a message-shaped skeleton covers intermediate initial-fill scroll states, final initial placement is applied instantly before real content is revealed on the next animation frame, mark-read/active-read/jump-to-latest/boundary-load/viewport-fill/latest-autoscroll are gated until initial positioning is complete, `useChatScroll` no longer performs a surprise first auto-scroll when auto-scroll is re-enabled, and latest viewport auto-fill preserves position when prepending older rows; backend/API/SDK/DB, reply navigation redesign, virtualization, link previews, and desktop release validation remain unchanged
