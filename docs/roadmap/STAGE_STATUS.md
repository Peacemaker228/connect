...
- `customer-reply-navigation-loaded-range-polish` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_218_CUSTOMER_REPLY_NAVIGATION_LOADED_RANGE_POLISH.md`: clicking a sent reply preview should scroll to the original message when the original is already loaded in the current channel/DM view, briefly highlight the target, and avoid history fetch/deep-link/backend/realtime/unread changes
...
- implement Segment 218 loaded-range reply navigation, then smoke loaded-original scroll/highlight, deleted fallback, unloaded-original no-op, direct reply navigation, and edit/delete/copy regressions
...
- next focused reply follow-up after Segment 217 smoke: Segment 218 loaded-range reply navigation/scroll-to-original polish, unless reply attention smoke exposes a blocker
