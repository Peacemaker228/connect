...
- `customer-message-generic-file-attachments` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_212_CUSTOMER_MESSAGE_GENERIC_FILE_ATTACHMENTS.md`: backend `messageFile` now accepts generic files up to `50 MB`, `serverImage` remains image-only and `4 MB`, stored upload metadata preserves optional display filenames without a DB migration, frontend `messageFile` selection allows generic files, images keep inline preview, PDFs keep file-row/open behavior, other files render as generic download/open rows, non-image/non-PDF message files use attachment content disposition, and storage provider/env/secrets/bucket policy, DB schema/migrations, auth/session, unread/realtime, media/WebRTC, replies, link previews, multiple attachments, and `300 MB` large-file transfer remain unchanged
...
- run Segment 212 authenticated channel/direct smoke for image preview, PDF row/open, generic document row/open, archive row/download-only, executable-like row/download-only, `messageFile` over `50 MB` rejection, server image non-image rejection, screenshot paste, copy action, and reload restore
