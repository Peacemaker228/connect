...
- `customer-chat-anchor-context-history-navigation` is planned in `docs/delegation/briefs/SEGMENT_BRIEF_220_CUSTOMER_CHAT_ANCHOR_CONTEXT_HISTORY_NAVIGATION.md`: Segment 219's overlay is accepted as a bounded technical step but not the final UX model; the next reply/history slice should replace the overlay with an anchor/windowed history mode, support adjacent older/newer loading around the target, add a jump-to-latest/down control, avoid loading every message between old targets and latest, and keep full message-list virtualization as a later performance review only if profiling/smoke shows it is necessary
...
- implement Segment 220 anchor/windowed history navigation before reply visual redesign: unloaded reply targets should open a target-centered active history range, support older/newer adjacent loading, and provide a jump-to-latest/down control; do not solve the Segment 219 overlay gap with CSS spacing
