...
- create feature branches from latest `origin/core/reborn` without tracking `origin/core/reborn`; if upstream is wrong, clear it before handoff with `git branch --unset-upstream`;
...
- `customer-idle-profile-reconnect-sidebar-state-fix` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_197_CUSTOMER_IDLE_PROFILE_RECONNECT_SIDEBAR_STATE_FIX.md`: the profile query now recovers from anonymous `200` session snapshots by sharing the SDK refresh path, the server sidebar gates normal member/account rendering until profile and current member are ready, current-member recovery invalidates server-scoped unread summary, and no backend auth/session, unread realtime contract, DB, storage, media/WebRTC, or production/staging infra behavior was changed
- `customer-unread-sound-and-mute-settings` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_198_CUSTOMER_UNREAD_SOUND_AND_MUTE_SETTINGS.md`: accepted realtime unread events can play the local `public/sounds/that-was-quick-606.mp3` sound after existing own-message, active-chat, and duplicate guards pass, the account menu has a local `Notification sound` toggle backed by `localStorage`, and no Notification API, native desktop popup, app badge, DB, storage, media/WebRTC, or staging/prod infra change was made
- `customer-unread-per-chat-sound-mute-controls` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_199_CUSTOMER_UNREAD_PER_CHAT_SOUND_MUTE_CONTROLS.md`: channel and direct member rows now have compact hover/focus sound mute controls, muted scopes are stored locally under `ax-connect:unread-notification-muted-scopes`, visual unread badges/counts remain active while sound is blocked for muted scopes, and no backend preference persistence, DB, storage, media/WebRTC, or staging/prod infra change was made
- `customer-auth-session-idle-boundary-hardening` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_200_CUSTOMER_AUTH_SESSION_IDLE_BOUNDARY_HARDENING.md`: backend `GET /api/auth/session` now recovers expired access-cookie sessions through the existing refresh-cookie path, rotates cookies on success, returns a clear anonymous state without clearing cookies when recovery fails to avoid cross-tab refresh-token rotation races, and the client invalidates server/global unread plus server data when current-member identity recovers; `/api/auth/session/refresh`, SDK refresh-on-401, unread realtime contracts, DB, storage, media/WebRTC, and staging/prod infra remain unchanged
- `customer-unread-notification-pipeline-reliability` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_201_CUSTOMER_UNREAD_NOTIFICATION_PIPELINE_RELIABILITY.md`: active-chat unread/sound suppression is now visibility/focus-aware, hidden/minimized/unfocused active chats remain unread/sound eligible, sound playback returns reason-coded results, optional local diagnostics are available through `ax-connect:debug-unread-notifications`, and unread realtime contracts, backend/API, DB, storage, auth/session, media/WebRTC, and staging/prod infra remain unchanged
- `customer-chat-composer-screenshot-paste-and-focus-stability` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_202_CUSTOMER_CHAT_COMPOSER_SCREENSHOT_PASTE_FOCUS.md`: composer paste now detects clipboard image files, opens the existing `messageFile` confirmation modal with the pasted image preselected, reuses backend-owned staged storage plus message creation, and tightens safe composer focus on chat entry plus text/attachment send without changing backend/API, DB, storage provider config, auth/session, unread notifications, media/WebRTC, staging DB, or production infra
- `customer-unread-active-visible-read-semantics-fix` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_203_CUSTOMER_UNREAD_ACTIVE_VISIBLE_READ_SEMANTICS_FIX.md`: active visible near-bottom channel/DM events now auto mark-read without requiring `document.hasFocus()`, suppress server rail unread, row unread, and sound; active visible scrolled-up events keep visual unread/`New` state without sound; active hidden/minimized events remain unread/sound eligible unless muted; server sidebar active row badge hiding is tied to the active read boundary; unread backend/API contracts, DB, auth/session, storage, media/WebRTC, and staging/prod infra remain unchanged
- `customer-mentions-attention-foundation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_204_CUSTOMER_MENTIONS_ATTENTION_FOUNDATION.md`: channel messages now persist additive mention target rows, backend creation/update resolves stable `<@memberId>` / `<@all>` tokens plus current raw `@DisplayName` / `@all` text against server members, explicit self-mentions and `@all` self-target rows render/highlight while own messages still do not create own unread/sound, unread summaries and realtime promote only mentioned recipients to `attentionLevel: 'mention'`, message text renders mention chips with raw-token fallback, target users get subtle whole-message mention highlight, and normal unread, direct unread privacy, Segment 203 active-read semantics, reply-to-message, link rendering, message copy, storage/auth/media, and broad realtime transport hardening remain separate
- `customer-mentions-autocomplete-picker` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_205_CUSTOMER_MENTIONS_AUTOCOMPLETE_PICKER.md`: channel composer now has a bounded `@` picker backed by existing server member data only when the loaded server snapshot id matches the current `channelServerId`, uses existing shadcn/cmdk `Command` list primitives while keeping text input ownership in the chat textarea, includes a visually separated `@all` option that remains selectable when the member list exceeds the visible member cap, keeps the picker list in a Discord-like tall bounded `max-height` sized to fit the current cap of 8 member suggestions plus the separated `@all` option while shrinking on small viewports, keeps keyboard-selected options visible through native `scrollIntoView({ block: 'nearest' })` rather than custom scroll math, selected mentions remain readable in the textarea but submit as stable `<@memberId>` / `<@all>` tokens when the selected range is still intact, edited selected ranges fall back to Segment 204 raw parsing, and backend/API/SDK contracts, DB schema/migrations, unread/realtime behavior, auth/session, storage/media, replies, link rendering, message copy, and Notification API remain unchanged
- Mention picker scrollbar/pointer interactions are guarded so dragging the scroll bar does not close the picker through the textarea blur handler, and focus/caret are restored back to the chat textarea after pointer release.
- Mention picker polish debt is accepted for now: keyboard navigation can still visually jump because text input remains owned by the chat `textarea` while shadcn/cmdk owns only the popup list; revisit as a focused mention editor / combobox-controller task only if user feedback makes it worth the extra complexity.
- `customer-message-copy-action` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_206_CUSTOMER_MESSAGE_COPY_ACTION.md`: non-deleted channel/direct messages now expose a compact copy action available to non-owners, text and multiline messages copy readable content, metadata-backed stable mention tokens copy as `@name` / `@all`, image/PDF attachments copy useful backend access URLs without `[object Object]`, desktop uses `window.electron.writeClipboardText` when available while web uses `navigator.clipboard.writeText` with a legacy textarea fallback, and backend/API/SDK contracts, DB schema/migrations, storage config, auth/session, unread/realtime, media/WebRTC, links, previews, replies, and binary image clipboard writes remain unchanged
- mention model follow-up is documented in `PLATFORM_MIGRATION_PLAN.md`: future work should split mention rendering entities from notification/attention recipients; if self-mention after idle/reconnect shows yellow badges for other users until refresh, verify backend unread summaries first and treat realtime/cache reconciliation drift as the primary suspect
- `customer-message-edit-mode-correctness` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_207_CUSTOMER_MESSAGE_EDIT_MODE_CORRECTNESS.md`: message edit state is now controlled by the current `ChatMessages` view so only one row can be edited at a time, edit input autofocus places the caret at the end, stable mention tokens display as readable `@name` / `@all` in edit mode, and the existing message update mutation, backend/API contracts, DB schema, unread/realtime behavior, auth/session, storage/media, send button, replies, links, and broad file-transfer work remain unchanged
...
- Follow-up requirement: message edit mode should move to a textarea-style editor so multiline and long-message editing matches the main composer, while preserving autofocus/caret, Escape cancel, single-edit-mode, mention picker serialization, and existing save permissions.
...
- `customer-message-generic-file-attachments` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_212_CUSTOMER_MESSAGE_GENERIC_FILE_ATTACHMENTS.md`: backend `messageFile` now accepts generic files up to `50 MB`, `serverImage` remains image-only and `4 MB`, multipart filename mojibake is normalized for new Cyrillic filename uploads, stored upload metadata preserves optional display filenames without a DB migration, frontend `messageFile` selection allows generic files, upload copy reflects the single-attachment policy, file drop on the upload area/composer goes through the app upload/modal path instead of browser navigation, images keep inline preview, PDFs keep file-row/open behavior, other files render as generic download/open rows, non-image/non-PDF message files use attachment content disposition, and storage provider/env/secrets/bucket policy, DB schema/migrations, auth/session, unread/realtime, media/WebRTC, replies, link previews, multiple attachments, and `300 MB` large-file transfer remain unchanged
- `customer-file-upload-max-size-copy` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_213_CUSTOMER_FILE_UPLOAD_MAX_SIZE_COPY.md`: message attachment upload now shows translated visible max-size copy, `Maximum file size: 50 MB` / `РњР°РєСЃРёРјР°Р»СЊРЅС‹Р№ СЂР°Р·РјРµСЂ С„Р°Р№Р»Р°: 50 РњР‘`, before file selection; server image upload keeps separate server-image copy and does not show the `50 MB` message attachment limit; backend upload policy, accepted file types, storage provider/env/secrets, DB schema, auth/session, unread/realtime, media/WebRTC, replies, and mention navigation remain unchanged
- `customer-mention-chip-navigation` is implemented locally with command verification passing in `docs/delegation/briefs/SEGMENT_BRIEF_214_CUSTOMER_MENTION_CHIP_NAVIGATION.md`: metadata-backed single-user mention chips now navigate to existing direct conversations for other members, self mentions and `@all` remain non-clickable, raw fallback mentions without backend metadata remain non-clickable, and mention highlight, copy, edit, link rendering, backend/API/SDK contracts, DB schema, unread/realtime, auth/session, storage, media/WebRTC, replies, profile modal, and link previews remain unchanged
- `customer-reply-to-message-foundation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_215_CUSTOMER_REPLY_TO_MESSAGE_FOUNDATION.md`: channel and direct messages now have additive self-reference reply fields plus indexes, create payloads can include `replyToMessageId`, backend services validate reply targets inside the same channel/conversation, chat reads and existing created-message realtime payloads include a compact common `replyTo` preview, the chat UI has a local reply provider, non-deleted rows expose `Reply`, composers show a cancelable reply bar, sent messages render compact inline reply previews, existing update realtime refreshes loaded reply previews when the original message is edited or soft-deleted, selecting `Reply` returns focus to the composer, deleted originals render a safe fallback through the existing soft-delete path, and reply attention/badges/sound, `replyCount` increments, scroll-to-original, visual redesign, partial quotes, threads, link previews, auth/storage/media changes, and staging/prod migration execution remain deferred
- `customer-reply-preview-mention-rendering-polish` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_216_CUSTOMER_REPLY_PREVIEW_MENTION_RENDERING_POLISH.md`: compact reply previews now reuse shared mention text splitting so metadata-backed `@user` and `@all` render as readable highlighted non-interactive spans instead of raw stable tokens, normal message content keeps its existing mention navigation behavior, deleted-original and file-only reply fallbacks remain unchanged, and backend/API/DB/realtime contracts, reply attention/badges/sound, `replyCount`, scroll-to-original, and reply block redesign remain deferred
- `customer-reply-attention-unread-foundation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_217_CUSTOMER_REPLY_ATTENTION_UNREAD_FOUNDATION.md`: backend summaries now count channel/direct unread replies to the current member's messages, channel unread realtime includes `repliedToMemberId` for member-specific client promotion, direct unread remains recipient-private, scoped/global caches aggregate member-specific `replyCount`, `attentionLevel` keeps `mention > reply > unread > none`, reply attention has a subtle sky accent in server/channel/member badges, active-read and mute behavior are preserved, and DB schema/migrations, new event types, scroll-to-original, thread UI, and reply block redesign remain unchanged
- `customer-reply-navigation-loaded-range-polish` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_218_CUSTOMER_REPLY_NAVIGATION_LOADED_RANGE_POLISH.md`: sent reply previews now use a `ChatMessages`-owned loaded-row map to scroll to an original message already rendered in the current channel/DM view, apply a short inset highlight without layout shift, quietly no-op when the original is not loaded, keep deleted targets and the composer reply bar non-navigating, and avoid backend/API/DB/realtime/unread, history fetch, deep-link, thread UI, and reply block redesign changes
- `customer-reply-target-context-navigation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_219_CUSTOMER_REPLY_TARGET_CONTEXT_NAVIGATION.md`: reply previews keep the Segment 218 loaded-row fast path and now fetch bounded backend-owned context when the target original is outside the rendered range, using `GET /api/messages/:messageId/context` or `GET /api/direct-messages/:directMessageId/context`, SDK helper `fetchChatReplyTargetContext()`, a chat-local non-duplicate context overlay that preserves normal infinite-query pagination, realtime patching of context-loaded rows and their reply previews, and post-mount scroll/highlight; soft-deleted targets can navigate to existing deleted fallback rows when accessible, while reply visual redesign, reply attention/unread changes, DB schema/migrations, realtime events, auth/storage/media changes, deep links, and unbounded "load previous" loops remain out of scope
- `customer-chat-anchor-context-history-navigation` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_220_CUSTOMER_CHAT_ANCHOR_CONTEXT_HISTORY_NAVIGATION.md`: Segment 219's overlay is replaced by anchored history mode for unloaded reply targets, context endpoints now return `olderCursor` / `newerCursor` and accept `direction=older|newer` for bounded adjacent history reads, initial target context uses the normal bounded page size on each side, chat rows now render in natural chronological DOM order instead of `flex-col-reverse`, normal latest infinite-query behavior remains intact, latest/anchored ranges auto-fill underfilled viewports with bounded follow-up reads, older/newer loading triggers near the viewport boundary and preserves viewport position after prepending rows with pre-paint scroll correction, programmatic loaded-target smooth scrolling briefly suppresses boundary pagination, reply targets use deterministic container-relative centering with native smooth for nearby loaded targets, short fake-smooth jumps for far loaded targets, and instant highlighted positioning for newly loaded context targets, latest-mode auto-scroll/forced-bottom scrolling is disabled while anchored, anchored history shows the welcome/start state at the beginning, anchored rows/reply previews keep realtime update/delete patching, and a sticky down control appears whenever the user is away from the live bottom, using smooth/fake-smooth movement in normal loaded latest ranges, deterministic positioning for anchored window switches, and automatic anchored-mode exit once the live bottom has been reached; virtualization, reply visual redesign, reply attention/unread changes, DB schema/migrations, auth/storage/media/WebRTC, realtime transport hardening, deep links, and thread UI remain out of scope
- `customer-reply-telegram-visual-polish` is implemented locally in `docs/delegation/briefs/SEGMENT_BRIEF_221_CUSTOMER_REPLY_TELEGRAM_VISUAL_POLISH.md`: shared reply previews now use a compact Telegram-like violet accent line and subtle violet background, the composer reply bar inherits the same shared style, reply target navigation highlight now uses a temporary violet background instead of the previous orange ring-style treatment, double-clicking an empty area of a non-deleted message row starts a reply while text/links/files/avatars/headers/reply previews/actions/edit controls are ignored, Escape closes active composer reply mode, and the jump-to-latest arrow is forced to a stable square size; Segment 220 navigation behavior, metadata-backed reply preview mentions, backend/API/SDK/DB/realtime/auth/storage/media/WebRTC, reply attention/unread semantics, virtualization, deep links, and thread UI remain unchanged
  - `customer-chat-initial-scroll-stability-fix` is implemented locally with command verification and targeted local smoke passing in `docs/delegation/briefs/SEGMENT_BRIEF_222_CUSTOMER_CHAT_INITIAL_SCROLL_STABILITY_FIX.md`: chat entry now has a single initial-scroll owner before normal read/scroll side effects are enabled, channel/direct message list endpoints accept a bounded `limit` parameter, the chat client uses a viewport-estimated initial limit so desktop panes do not start from an underfilled fixed page, server-entry chat prefetch uses a larger bounded limit, unread chats can perform bounded older-page loading before settling on the first loaded unread non-own message, chats without unread settle at latest bottom, initial latest windows load bounded older pages when rendered rows are shorter than the viewport, a message-shaped skeleton covers intermediate cold-load initial-fill scroll states, repeat navigation with valid React Query chat cache renders cached messages immediately only after unread summary is known and there is no known unread target, while the active chat query still refetches on mount/return to reconcile messages created while the user was away; known-unread warm-cache returns wait for latest-page reconciliation before final reveal to avoid stale-cache jumps, final initial placement is applied instantly before real content is revealed on cold load, mark-read/active-read/jump-to-latest/boundary-load/viewport-fill/latest-autoscroll are gated until initial positioning is complete, `useChatScroll` no longer performs a surprise first auto-scroll when auto-scroll is re-enabled, user scroll intent cancels pending delayed scroll-to-bottom / initial-target corrections so immediate post-entry scroll-up does not snap back to latest, older-history top-boundary loading is scroll-direction aware and slightly debounced to avoid prepending old pages after a brief threshold crossing while the user scrolls back down, older-page prepend compensation now preserves the nearest visible message row as an anchor when async pages arrive, jump-to-latest renders outside scroll content so its visibility does not change `scrollHeight`, latest viewport auto-fill preserves position when prepending older rows, and targeted local smoke covered latest-bottom entry, immediate post-entry scroll-up, upward older-page prepend, and down-scroll without extra older-page requests; DB/schema/migrations, reply navigation redesign, virtualization, link previews, and desktop release validation remain unchanged
  - `customer-chat-scrollbar-and-new-below-jump-badge` is implemented locally with command verification and targeted local smoke passing in `docs/delegation/briefs/SEGMENT_BRIEF_223_CUSTOMER_CHAT_SCROLLBAR_AND_NEW_BELOW_JUMP_BADGE.md`: chat scroll containers now use a subtle native scrollbar utility, shared shadcn/Radix `ScrollArea` scrollbars are thinner and quieter for server/channel/member/sidebar-style lists, and the floating down control gets a local `99+`-capped badge when incoming non-own messages arrive below the current viewport in latest-mode chats; badge color follows existing attention priority `mention > reply > unread`, old-history prepends/initial load/own messages do not increment it, first click with unseen new messages scrolls to the first unseen message using a short fake-smooth movement for far first-new jumps, and the next click still goes to live bottom if the user remains away from latest; targeted local smoke confirmed ordinary incoming badge `1`, accumulated badge `3`, mention-priority amber badge, real backend/realtime delivery from a second authenticated user, old-history prepend negative case, and tall lower-batch first-new-then-latest two-step behavior; backend/API/SDK/DB/realtime contracts, persisted unread state, desktop native badges, virtualization, reply navigation redesign, link previews, and WebRTC/media remain unchanged
  - Follow-up requirement: progressive read decrement must make jump-anchor, server rail, channel row, member row, tab title, and other unread badges decrease as the user scrolls through newly visible messages, with attention color downgrading as mention/reply items are consumed and backend unread summaries remaining the reconciliation source of truth.

Next likely work:
- deploy and smoke Segment 221 plus Segment 222 on staging, then prioritize a global desktop staging/release validation pass before starting lower-priority product polish;
...
- run authenticated web smoke for Segment 202 channel/DM screenshot paste, cancel cleanup, text paste, plus-button upload, Enter/Shift+Enter, and focus stability; follow with packaged desktop runtime review if this moves toward desktop pass
- run two/three-user mention attention smoke for plain unread, `@user`, explicit self-mention rendering/highlight, `@all` including sender rendering/highlight, sender own-message unread/sound negative case, read clearing, reload restore, hidden/scrolled-up behavior, muted channel visual attention, and desktop runtime review
- run two/three-user Segment 205 mention picker smoke for channel picker open/filter/select, selected `@user`, selected `@all`, selected self mention, duplicate display names through stable selected tokens, keyboard behavior, emoji/screenshot/attachment compatibility, reload restore, mention attention, sender own-message unread/sound negative case, and desktop runtime review
- run Segment 207 authenticated web smoke for ordinary text edit, multiline edit, `@user`, `@all`, Escape cancel, unchanged mention save, single-edit switching, owner/non-owner permissions, delete action, and desktop runtime review
- run Segment 208 authenticated channel smoke for edit picker `@user`, edit picker `@all`, duplicate display-name selected member A/B, raw typed `@name` limitation, collision-aware placement near viewport top/bottom, Escape cancel, switching edited message, composer picker regression, and desktop runtime review
- run Segment 209 authenticated channel/direct smoke for button send, Enter send, Shift+Enter newline, empty disabled state, rapid clicks, focus restore, emoji insert, mention picker, screenshot paste, plus upload, and narrow-width layout
- run Segment 210 authenticated channel/direct smoke for `http://`, `https://`, `www.`, multiple links, trailing punctuation, multiline text, mention plus link, deleted-message regression, attachment regression, and copy regression
- run Segment 212 authenticated channel/direct smoke for image preview, PDF row/open, generic document row/open, Cyrillic filename display, archive row/download-only, executable-like row/download-only, `messageFile` over `50 MB` rejection, server image non-image rejection, screenshot paste, file upload area drag/drop, composer drag/drop, copy action, and reload restore
...
- run Segment 216 authenticated smoke for reply previews containing `@user`, `@all`, edited-original mention updates, deleted-original fallback, file-only fallback, normal mention chip navigation regression, and direct-message plain reply preview regression
- run Segment 217 smoke for 3-user channel reply attention, mention-over-reply priority, sender negative case, direct reply attention, active-visible read clearing, muted visual attention, reload restore, and reconnect/focus duplicate sanity before any reply navigation polish
- run Segment 218 smoke for loaded-original scroll/highlight, deleted fallback, unloaded-original no-op, direct reply navigation, reply preview mention rendering, and edit/delete/copy regressions
- run Segment 219 smoke for loaded/unloaded/deleted channel and DM reply targets, wrong-chat API rejection, no duplicate rows, and edit/delete/copy/mention preview regressions
- run Segment 220 smoke for loaded/unloaded/deleted channel and DM reply targets, anchored older/newer loading, jump-to-latest, realtime new message while anchored, wrong-chat rejection, and edit/delete/copy/mention/file regressions
- run Segment 221 visual smoke for channel/DM reply previews, `@user` / `@all` in reply previews, deleted-original fallback, loaded/unloaded target violet highlight, composer reply bar select/cancel/send/Escape, double-click empty-area reply, double-click ignored controls/text, jump-arrow size, and light/dark theme contrast
...
- `desktop-auto-update-flow-hardening` is implemented locally and awaiting packaged smoke in `docs/delegation/briefs/SEGMENT_BRIEF_231A_DESKTOP_AUTO_UPDATE_FLOW_HARDENING.md`: updater status now includes `unsupported`, safe timestamp/error/progress fields, concurrent-check guards, explicit safe install behavior, manual retry after error/no-update, and a 45-minute packaged-staging periodic check; no update-ready UX polish, production provider, signing, or CI/CD was added.
...
- auto-update implementation is proven for packaged staging `0.0.3 -> 0.0.4`, and updater lifecycle hardening is implemented locally pending packaged smoke; the process is still manual, not backed by CI/CD, and the visible update UX is minimal;
...
Next likely work:
- smoke `desktop-auto-update-flow-hardening`, then continue to `desktop-update-ready-ux-polish`; after that, continue desktop file-download UX polish and production release hardening while monitoring staging user feedback on native/web notification behavior. The release track still needs to move the proven manual auto-update path toward a repeatable CI/operator release process.
