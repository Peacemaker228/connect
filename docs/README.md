# Docs Index

## Main Entry Points

- [roadmap/PLATFORM_MIGRATION_PLAN.md](./roadmap/PLATFORM_MIGRATION_PLAN.md) — главный архитектурный и миграционный план
- [roadmap/ARCHITECTURE.md](./roadmap/ARCHITECTURE.md) — целевая архитектурная модель
- [roadmap/BOUNDARIES.md](./roadmap/BOUNDARIES.md) — границы auth/storage/media/realtime/domain
- [roadmap/STAGE_STATUS.md](./roadmap/STAGE_STATUS.md) — что уже сделано по стадиям и что осталось
- [waves/FIRST_MIGRATION.md](./waves/FIRST_MIGRATION.md) — текущая первая волна миграции

## Delegation

- [delegation/DELEGATION_AGENT_GUIDE.md](./delegation/DELEGATION_AGENT_GUIDE.md) — как должен работать делегируемый исполнитель
- [delegation/briefs/SEGMENT_BRIEF_001_APP_CORE_SEED.md](./delegation/briefs/SEGMENT_BRIEF_001_APP_CORE_SEED.md)
- [delegation/briefs/SEGMENT_BRIEF_002_SDK_SEED.md](./delegation/briefs/SEGMENT_BRIEF_002_SDK_SEED.md)
- [delegation/briefs/SEGMENT_BRIEF_003_UI_SEED.md](./delegation/briefs/SEGMENT_BRIEF_003_UI_SEED.md)

## Recommended Reading Order

1. [roadmap/PLATFORM_MIGRATION_PLAN.md](./roadmap/PLATFORM_MIGRATION_PLAN.md)
2. [roadmap/STAGE_STATUS.md](./roadmap/STAGE_STATUS.md)
3. [waves/FIRST_MIGRATION.md](./waves/FIRST_MIGRATION.md)
4. [delegation/DELEGATION_AGENT_GUIDE.md](./delegation/DELEGATION_AGENT_GUIDE.md)
5. нужный `SEGMENT_BRIEF`
